...
- (✨) A common construction in modal logic is to take the transitive closure $\Box^\ast$ of an operator $\Box$; the complete logic of $\Box^\star$ is then axiomatized using a transitive closure rule and an induction rule.  What about the *transitive closure,* i.e. deriving a minimal $\Box^-$ from $\Box^\ast$?  I'm especially interested in this within a *dynamic* context:  Given $[P]^\ast$, what is the complete logic of $[P], [P]^\ast, [P]^-$?
