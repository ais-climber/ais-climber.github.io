...
- [📝](/notes_on_tcfgs/) (TODO) Given a tournament, a Tiered Coalition Formation Game is the task of making a tier list 

we can make a tier list that resolves all the cycles in a stable way (this is called a "tiered coalition formation game", see TODO).  There are many different ways to define stability -- one is "local stability," which asserts that swapping any two agents in the tier list results in a *worse* `#forward edges - #backward edges`.  Is local stability polynomial time for weighted tournaments?
